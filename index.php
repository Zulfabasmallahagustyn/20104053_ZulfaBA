...
function beliProduk($produkId, $quantity)
{
    global $conn;
    $userId = $_SESSION['user_id'];

    // Mendapatkan data produk
    $query = "SELECT * FROM produk WHERE produk_id = $produkId";
    ...
    if ($produk['stok'] >= $quantity) {
        ...
        $updateQuery = "UPDATE produk SET stok = $newStok WHERE produk_id = $produkId";
        $conn->query($updateQuery);

        // Menyimpan data transaksi
        $insertQuery = "INSERT INTO transaksi (user_id, produk_id, quantity, tanggal, total_harga) VALUES ($userId, $produkId, $quantity, NOW(), $totalHarga)";
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav ml-auto">
          <li class="nav-item">
            ...
            ?> <li class="nav-item">
            <a class="nav-link" href="admin/index.php">Product</a>
          </li>
        </ul>
      </div>
    </nav>
